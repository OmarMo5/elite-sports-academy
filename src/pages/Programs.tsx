...
const HeroSection = () => (
  <section className="relative min-h-[50vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary via-primary/90 to-foreground">
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {["🏊", "🎾", "⚽", "🏀", "🤸", "🥋"].map((icon, i) => (
        <span key={i} className={`absolute text-4xl md:text-6xl opacity-10 ${i % 2 === 0 ? "animate-float" : "animate-float-reverse"}`} style={{ top: `${15 + i * 12}%`, left: `${5 + i * 15}%`, animationDelay: `${i * 0.5}s` }}>
          {icon}
        </span>
      ))}
    </div>
    <div className="container mx-auto px-4 text-center relative z-10 pt-24 pb-16">
      ...
      <p className="text-primary-foreground/70 text-lg max-w-2xl mx-auto font-body">
        World-class training across 6 sports disciplines, designed for athletes of every age and skill level.
      </p>
    </div>
...
const ProgramsGrid = () => {
  ...
  const skillOptions = ["All", "Beginner", "Intermediate", "Advanced", "Competitive"];
        ...
        <div className="flex flex-wrap items-center gap-2 mb-10 justify-center">
          ...
          {skillOptions.map((opt) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-subheading font-medium transition-all ${filter === opt ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:bg-primary/10 hover:text-primary"}`}
            >
              {opt}
            </button>
          ))}
          ...
          {filtered.map((sport, i) => (
            ...
            >
              <Card className="overflow-hidden border-border/50 hover:border-primary/30 transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 bg-card group-hover:scale-[1.02]" style={{ transformStyle: "preserve-3d" }}>
                {/* Colored top bar */}
                <div className={`h-1.5 bg-${sport.color}`} />
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground flex items-center gap-1"><Zap className="w-3.5 h-3.5" /> Intensity</span>
