...
/* ─── Hero ─── */
const Hero = () => (
  <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary via-primary/90 to-foreground">
    {/* Floating icons */}
    ...
    <div className="container mx-auto px-4 text-center relative z-10 pt-20">
      ...
      <p className="text-primary-foreground/70 text-lg md:text-xl max-w-2xl mx-auto mb-10 font-body">
        World-class coaching in swimming, tennis & 6+ sports. Join 2,500+ students training across 8 branches.
      </p>
    ...
    <section className="py-20 bg-secondary/50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground mt-2 font-subheading">A decade of building champions</p>
        </div>
        <div ref={ref} className="relative max-w-3xl mx-auto">
          ...
          {milestones.map((m, i) => (
            <div
              key={m.year}
              className={`relative flex items-start mb-10 transition-all duration-500 ${
...
const SportsSection = () => {
  ...
  return (
    <section className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="font-heading font-bold text-3xl md:text-4xl text-foreground">Our Sports Programs</h2>
          <p className="text-muted-foreground mt-2 font-subheading">World-class training across multiple disciplines</p>
        </div>
        <div ref={ref} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {sports.map((sport, i) => (
            <div
              ...
              style={{ transitionDelay: `${i * 100}ms`, perspective: "1000px" }}
            >
              <Card className="overflow-hidden border-border/50 hover:border-primary/30 transition-all duration-300 hover:shadow-xl hover:-translate-y-2 bg-card"
...
const EventsSection = () => {
  ...
  return (
    <section className="py-20 bg-secondary/50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="font-heading font-bold text-3xl md:text-4xl text-foreground">Upcoming Events</h2>
          <p className="text-muted-foreground mt-2 font-subheading">Competitions and celebrations ahead</p>
        </div>
        <div ref={ref} className={`relative max-w-2xl mx-auto transition-all duration-700 ${isVisible ? "opacity-100" : "opacity-0"}`}>
          <Card className="overflow-hidden border-border/50 bg-card">
            <CardContent className="p-8 text-center min-h-[200px] flex flex-col justify-center">
              <span className="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-subheading font-semibold mb-3 mx-auto">
                {upcomingEvents[current].sport}
              </span>
          ...
          <div className="flex items-center justify-center gap-4 mt-6">
            <button
              onClick={() => setCurrent((c) => (c - 1 + upcomingEvents.length) % upcomingEvents.length)}
            ...
            <button
              onClick={() => setCurrent((c) => (c + 1) % upcomingEvents.length)}
              className="w-10 h-10 rounded-full border border-border flex items-center justify-center hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all"
            >
...
const TestimonialsSection = () => {
  ...
  return (
    <section className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="font-heading font-bold text-3xl md:text-4xl text-foreground">What Families Say</h2>
          <p className="text-muted-foreground mt-2 font-subheading">Trusted by thousands of parents across Egypt</p>
        </div>
        <div ref={ref} className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {testimonials.map((t, i) => (
            <Card
              key={i}
              className={`border-border/50 hover:border-gold/30 hover:shadow-lg transition-all duration-500 bg-card ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
              }`}
              style={{ transitionDelay: `${i * 150}ms` }}
            >
  ...
  return (
    <section className="py-20 bg-gradient-to-r from-primary to-primary/80 relative overflow-hidden">
      <div className="absolute inset-0 opacity-10">
        {["🏊", "🎾", "⚽"].map((icon, i) => (
          <span key={i} className="absolute text-8xl animate-float-slow" style={{ top: `${20 + i * 25}%`, right: `${5 + i * 10}%`, animationDelay: `${i}s` }}>
            {icon}
          </span>
        ))}
      </div>
      <div ref={ref} className={`container mx-auto px-4 text-center relative z-10 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        ...
        <p className="text-primary-foreground/70 max-w-xl mx-auto mb-8 font-body">
          Join 2,500+ students across 8 branches. Find the perfect program for your child today.
        </p>
        <Link to="/membership">
