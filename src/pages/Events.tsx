    ...
    <section className="relative min-h-[50vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary via-primary/90 to-foreground">
      <div className="container mx-auto px-4 text-center relative z-10 pt-24 pb-16">
        <h1 className="font-heading font-black text-4xl sm:text-5xl md:text-6xl text-primary-foreground leading-tight mb-4">
          {t("events.heroTitle")} <span className="text-gold">{t("events.heroHighlight")}</span>
        </h1>
        <p className="text-primary-foreground/70 text-lg max-w-2xl mx-auto font-body">{t("events.heroSubtitle")}</p>
      </div>
      ...
      <section className="py-16 bg-background">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-3 gap-8 max-w-md mx-auto">
            <MedalStat label={t("events.goldMedals")} value={totalGold} color="text-gold" />
            <MedalStat label={t("events.silverMedals")} value={totalSilver} color="text-muted-foreground" />
            <MedalStat label={t("events.bronzeMedals")} value={totalBronze} color="text-accent-red" />
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <Tabs defaultValue="upcoming" className="w-full">
            <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 mb-10">
              <TabsTrigger value="upcoming" className="font-subheading">{t("events.upcoming")}</TabsTrigger>
              <TabsTrigger value="past" className="font-subheading">{t("events.past")}</TabsTrigger>
            </TabsList>
                  ...
                  <Card key={event.id} className={`border-border/50 hover:border-primary/30 hover:shadow-lg transition-all duration-500 bg-card ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} style={{ transitionDelay: `${i * 100}ms` }}>
                    <CardContent className="p-6" dir={isRTL ? "rtl" : "ltr"}>
                      <div className="flex flex-col md:flex-row md:items-center gap-4">
                        <div className="w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><Calendar className="w-6 h-6 text-primary" /></div>
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-4 mt-2 text-xs text-muted-foreground">
                            ...
                            <span className="flex items-center gap-1"><Users className="w-3 h-3" />{event.participants} {t("events.participants")}</span>
                          </div>
                        </div>
                      </div>
                    </CardContent>
              ...
              <div className="space-y-4">
                {pastEvents.map((event) => (
                  <Card key={event.id} className="border-border/50 hover:border-gold/30 hover:shadow-lg transition-all duration-300 bg-card">
                    <CardContent className="p-6" dir={isRTL ? "rtl" : "ltr"}>
                      <div className="flex flex-col md:flex-row md:items-center gap-4">
                        <div className="w-14 h-14 rounded-xl bg-gold/10 flex items-center justify-center shrink-0"><Trophy className="w-6 h-6 text-gold" /></div>
                        <div className="flex-1">
                        ...
                        {event.results && (
                          <div className={`flex gap-3 ${isRTL ? "md:border-r md:border-border md:pr-4" : "md:border-l md:border-border md:pl-4"}`}>
