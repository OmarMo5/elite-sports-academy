...
const galleryColors = [
  ...
  "from-accent-red/20 to-primary/10",
];
  ...
  <section className="relative min-h-[45vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary via-primary/90 to-foreground">
    <div className="container mx-auto px-4 text-center relative z-10 pt-24 pb-16">
      ...
      <p className="text-primary-foreground/70 text-lg max-w-2xl mx-auto font-body">
        Capturing excellence — moments from training, competitions, and celebrations.
      </p>
    </div>
...
const GalleryPage = () => {
  ...
  const categories = ["All", "Training", "Competition", "Awards"];
    ...
    <main>
          ...
          <div className="flex flex-wrap items-center gap-2 mb-10 justify-center">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-subheading font-medium transition-all ${filter === cat ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:bg-primary/10 hover:text-primary"}`}
              >
                {cat}
              </button>
            ))}
          ...
          <div ref={ref} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map((img, i) => (
              <div
                ...
                className={`group relative aspect-[4/3] rounded-xl overflow-hidden cursor-pointer transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
                style={{ transitionDelay: `${i * 60}ms`, perspective: "800px" }}
              >
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 z-50 bg-foreground/90 flex items-center justify-center p-4" onClick={() => setLightbox(null)}>
          <button className="absolute top-6 right-6 w-10 h-10 rounded-full bg-background/20 flex items-center justify-center text-background hover:bg-background/40 transition-all">
            <X className="w-5 h-5" />
          </button>
